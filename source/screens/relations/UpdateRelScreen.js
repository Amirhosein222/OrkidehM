...
const UpdateRelScreen = ({ navigation, route }) => {
  ...
  const [partner, setPartner] = useState(params.rel.woman_name);
  const [partnerMobile, setPartnerMobile] = useState(params.rel.woman.mobile);
  const [picture, setPicture] = useState(
    params.rel.woman_image ? baseUrl + params.rel.woman_image : '',
  );
  ...
  const onUpdateRel = async (id) => {
    ...
    formData.append('woman_name', partner);
